$(document).ready(function () {
    ...
    $("#searchBtn").click(function () {
        ...
        pageHistory(
            `http://140.118.121.110:5534/history?startTime=${startTime}%2B08:00&endTime=${endTime}%2B08:00&cellId=${cellId}&deviceId=${iotId}`
        );
    });
    ...
    function initial() {
        getResource();
        let startTime = new Date(Date.now() - 16 * 3600000)
            .toISOString()
            .substr(0, 16);
        let endTime = new Date(Date.now() + 8 * 3600000)
            .toISOString()
            .substr(0, 16);
        $("#startTime").val(startTime);
        $("#endTime").val(endTime);
        pageHistory(
            `http://140.118.121.110:5534/history?startTime=${startTime}%2B08:00&endTime=${endTime}%2B08:00&cellId=&deviceId`
        );
        getDeviceId();
    }

    function getDeviceId() {
        $.get("http://140.118.121.110:5534/deviceId", function (data) {
            let dataHtml = `<option value="">Select IoT ID</option>`;
            for (i = 0; i < JSON.parse(data).deviceId.length; i++) {
                dataHtml += `<option value="${JSON.parse(data).deviceId[i]}">${
                    JSON.parse(data).deviceId[i]
                }</option>`;
            }
